...
import { 
  ...
  ShieldAlert, ArrowDownUp, Info, AlertTriangle, User, MessageSquareHeart
} from 'lucide-react';
                ...
                <ScrollArea className="max-h-[60vh]">
                    ...
                    {!isLoading && filteredDrugs.map((drug) => (
                        ...
                        >
                           ...
                           {drug.meta_data.nombres_comerciales && drug.meta_data.nombres_comerciales.length > 0 && (
  ...
  return (
    <div className="max-w-7xl mx-auto animate-in fade-in duration-500 pb-20">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                      ...
                      <div className="relative z-10 flex flex-col md:flex-row items-start justify-between gap-8">
                          <div className="flex-1">
                              <div className="flex flex-wrap gap-3 mb-4">
                                  {drug.meta_data.grupo_farmacologico && <Badge variant="secondary" className="bg-accent/20 text-accent border-accent/30 font-bold uppercase tracking-widest text-[10px]">{drug.meta_data.grupo_farmacologico}</Badge>}
                                  {drug.meta_data.status_regulatorio && <Badge variant="outline" className="border-white/10 text-slate-400">{drug.meta_data.status_regulatorio}</Badge>}
                              </div>
                              ...
                              {drug.meta_data.nombres_comerciales && drug.meta_data.nombres_comerciales.length > 0 && <p className="text-lg text-muted-foreground font-medium italic opacity-80 break-words">{drug.meta_data.nombres_comerciales.join(" • ")}</p>}
                          </div>
              ...
              {drug.resumen_clinico && (drug.resumen_clinico.inicio_accion || drug.resumen_clinico.duracion_efecto) && (
                <div className="lg:col-span-12 grid grid-cols-1 md:grid-cols-2 gap-6">
                    {drug.resumen_clinico.inicio_accion && (
                        <div className="bg-emerald-500/10 border border-emerald-500/20 p-6 rounded-4xl flex items-center gap-6">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-black text-white">{drug.resumen_clinico.inicio_accion}</p>
                            </div>
                        </div>
                    )}
                    {drug.resumen_clinico.duracion_efecto && (
                        <div className="bg-blue-500/10 border border-blue-500/20 p-6 rounded-4xl flex items-center gap-6">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-black text-white">{drug.resumen_clinico.duracion_efecto}</p>
                            </div>
                        </div>
                    )}
                </div>
              ...
              <div className="lg:col-span-12 mt-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-8">
                        {(drug.farmacologia_clinica?.mecanismo_accion || drug.resumen_clinico?.puntos_clave?.length > 0) && (
                            <PinterestCard className="h-fit">
                                ...
                                <div className="space-y-6">
                                    {drug.farmacologia_clinica?.mecanismo_accion && (
                                    ...
                                    {drug.resumen_clinico?.puntos_clave && drug.resumen_clinico.puntos_clave.length > 0 && (
                                        <div>
                        ...
                        {drug.seguridad_y_alertas?.monitoreo_recomendado && drug.seguridad_y_alertas.monitoreo_recomendado.length > 0 && (
                            <PinterestCard className="border-orange-500/20 h-fit">
                                ...
                                <div className="space-y-6">
                                    ...
                                    {drug.seguridad_y_alertas.instrucciones_manejo && (
                                        <div className="bg-slate-500/5 p-5 rounded-3xl border border-slate-500/10">
                                            ...
                                            <p className="text-sm text-slate-300 leading-relaxed italic">{drug.seguridad_y_alertas.instrucciones_manejo}</p>
                                        </div>
                                    )}
                                </div>
                            </PinterestCard>
                        ...
                        {drug.seguridad_y_alertas?.contraindicaciones_especie && Object.keys(drug.seguridad_y_alertas.contraindicaciones_especie).length > 0 && (
                                ...
                                <div className="space-y-4">
                                    {Object.entries(drug.seguridad_y_alertas.contraindicaciones_especie).map(([sp, msg]) => (
                                        <div key={sp} className="bg-red-500/10 border border-red-500/20 p-4 rounded-2xl">
                                            ...
                                            <p className="text-sm text-red-200 mt-1">{msg as string}</p>
                                        </div>
                                    ))}
                                </div>
                            </PinterestCard>
                    ...
                    <div className="space-y-8">
                        {drug.seguridad_y_alertas && (drug.seguridad_y_alertas.contraindicaciones?.length > 0 || drug.seguridad_y_alertas.efectos_adversos?.length > 0) && (
                            <PinterestCard className="border-red-500/20 h-fit">
                                ...
                                <div className="space-y-6">
                                    {drug.seguridad_y_alertas.contraindicaciones && drug.seguridad_y_alertas.contraindicaciones.length > 0 && (
                                    ...
                                    {drug.seguridad_y_alertas.efectos_adversos && drug.seguridad_y_alertas.efectos_adversos.length > 0 && (
                                        <div className={cn(drug.seguridad_y_alertas.contraindicaciones?.length > 0 && 'pt-4 border-t border-white/5')}>
                        ...
                        {drug.seguridad_y_alertas?.interacciones_farmacologicas && drug.seguridad_y_alertas.interacciones_farmacologicas.length > 0 && (
              ...
              {drug.parametros_dosificacion && Object.keys(drug.parametros_dosificacion).length > 0 && (
                <div className="lg:col-span-12 mt-4">
                                        ...
                                        {doses.map((d, i) => (
                                            <div key={i} className="bg-background/60 backdrop-blur-md p-5 rounded-3xl border border-white/5 shadow-inner">
                                                <div className="flex justify-between items-start mb-3 gap-2">
                                                    {d.indicacion && <span className={`text-[10px] font-black uppercase tracking-widest flex-1 ${theme.text}`}>{d.indicacion}</span>}
                                                    {d.vias && <Badge variant="outline" className="text-[9px] uppercase border-white/10 text-slate-400 flex-shrink-0">
                                                        {Array.isArray(d.vias) ? d.vias.join(' / ') : d.vias}
                                                    </Badge>}
                                                </div>
                                                {d.math?.dosis_recomendada && (
                                                ...
                                                {d.math && (d.math.dosis_min !== undefined || d.math.dosis_max !== undefined) && (
              ...
              {drug.informacion_cliente && drug.informacion_cliente.length > 0 && (
                  <div className="lg:col-span-12">
